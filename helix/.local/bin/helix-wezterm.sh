...
case "$1" in
  ...
  "blame")
    split_pane_down
    echo "cd $pwd; tig blame $filename +$line_number; exit" | $send_to_bottom_pane
    ;;
  "check")
    split_pane_down
    case "$extension" in
  ...
  "fzf")
    split_pane_down
    echo "cd $pwd; helix-fzf.sh \$(rg --line-number --column --no-heading --smart-case . | fzf --delimiter : --preview 'bat --style=full --color=always --highlight-line {2} {1}' --preview-window '~3,+{2}+3/2' | awk '{ print \$1 }' | cut -d: -f1,2,3); exit" | $send_to_bottom_pane
    ;;
  "howdoi")
    split_pane_down
  ...
  "lazygit")
    ...
    if [ "$program" = "lazygit" ]; then
        ...
        echo "cd $pwd; lazygit; exit" | $send_to_bottom_pane
    fi
    ;;
  "open")
